...
def launch_account(
    account_name, place_id, password, port=open_config("RAM_port"), vip=False
):
    ...
    try:
        ...
        if response.status_code == 200 or 400:
            print(
                clr("G")
                + f"\nSuccessfully launched {account_name} to place {place_id}"
                + resetclr()
            )
            return 1
        else:
            print(
                clr("R")
                + f"\nFailed to launch account. Status code: {response.status_code}"
                + resetclr()
            )
            print(response.text)
            return 0
