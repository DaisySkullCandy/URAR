    ...
    for instances, pid in process_dict.items():
        try:
            ...
            if memory_usage_mb > threshold_mb:
                print(clr("Y") + f"PID {pid} exceeds {threshold_mb} MB ({memory_usage_mb:.2f} MB). Terminating..." + resetclr())
                # Open the process with PROCESS_TERMINATE access
                PROCESS_TERMINATE = 0x0001
                handle = ctypes.windll.kernel32.OpenProcess(PROCESS_TERMINATE, False, pid)
                if not handle:
                    print(clr("R") + f"Unable to open process with PID {pid}. Error code: {ctypes.windll.kernel32.GetLastError()}" + resetclr())
                    continue
                # Terminate the process
                result = ctypes.windll.kernel32.TerminateProcess(handle, 0)
                if result:
                    print(clr("G") + f"Process with PID {pid} terminated successfully." + resetclr())
                    return pid
                else:
                    print(clr("R") + f"Failed to terminate process with PID {pid}. Error code: {ctypes.windll.kernel32.GetLastError()}" + resetclr())
                    return 1
                # Close the handle
